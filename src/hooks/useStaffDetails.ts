      ...
      }),
    {
      getNextPageParam: (lastPage,) => {
        if (!!lastPage)
          ...
          else if (lastPage.staffMedia?.pageInfo?.hasNextPage || (!lastPage.characterMedia?.pageInfo?.hasNextPage && !lastPage.staffMedia))
            return {
              withInfo: false,
              withMedia: false,
              withStaffRole: true,
              page: (lastPage.staffMedia?.pageInfo?.currentPage || 0 as number) + 1,
            };
