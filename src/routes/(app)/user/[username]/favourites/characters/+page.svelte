<script lang="ts">
  ...
  import type { Maybe, CardCharactersVerticalOptions } from "@/types";
  import type { CharacterEdge } from "@/types/anilistv2";

  // Import Libs
  import { onMount } from "svelte";
  import useUserCharactersFavourites from "@/hooks/useUserCharactersFavourites";
  ...
  const userAnimeFavourites = useUserCharactersFavourites({
    args: {
      name: data.username,
    },
  });
  ...
  const edgesToCardMediaOptionsProps = (
    ...
    const cardOptions: CardCharactersVerticalOptions[] = [];
