<script lang="ts">
  ...
  import StaffVoiceActingRoleCard from "@/components/shared/StaffVoiceActingRoleCard.svelte";
  import StaffVoiceActingRoleCardSkeleton from "@/components/skeletons/StaffVoiceActingRoleCardSkeleton.svelte";
  ...
  let pages: Maybe<Maybe<Staff>[]> = null;
  ...
  onMount(() => {
    const handleScroll = useDebounce(() => {
      if (isBottomPage() && !isFetching && currentTab === "roles") {
        fetchNextPage();
      }
    }, 200);
  ...
  {#if isSuccess && !!pages}
    ...
    >
      ...
      <div class="w-full flex flex-col gap-6 lg:mt-4">
        <div
          class="flex flex-col items-center lg:items-start gap-y-2 px-4 lg:px-0 text-center"
        >
        ...
        <div class="flex flex-col" class:hidden={currentTab !== "about"}>
          ...
          {#if pages[0]?.dateOfBirth?.month}
            <div>
              <span class="font-medium mr-2">Birthday:</span>{pages[0]
                .dateOfBirth.day}/{pages[0].dateOfBirth.month}/{pages[0]
                .dateOfBirth.year}
            </div>
          {/if}
          {#if pages[0]?.age}
            <div>
              <span class="font-medium mr-2">Age:</span>{pages[0]?.age}
            </div>
          {/if}
          {#if pages[0]?.gender}
            <div>
              <span class="font-medium mr-2">Gender:</span>{pages[0].gender}
            </div>
          {/if}
          {#if pages[0]?.yearsActive}
            <div>
              <span class="font-medium mr-2">Years active:</span>{pages[0]
                .yearsActive[0] || "?"} - {pages[0].yearsActive[1] || "present"}
            </div>
          {/if}
          {#if pages[0]?.homeTown}
            <div>
              <span class="font-medium mr-2">Hometown:</span>{pages[0].homeTown}
            </div>
          {/if}
          {#if pages[0]?.bloodType}
            <div>
              <span class="font-medium mr-2">Blood Type:</span>{pages[0]
                .bloodType}
            </div>
          {/if}
          <p>
        ...
        <div
          ...
          class:hidden={currentTab !== "roles"}
        >
          {#each pages || [] as page}
            {#each page?.characterMedia?.edges || [] as edge}
              {#if edge?.characters && edge.characters[0]}
                <StaffVoiceActingRoleCard type="characterMedia" source={edge} />
              {/if}
            {/each}
            {#each page?.staffMedia?.edges || [] as edge}
              <!-- {#if media?.characters && media.characters[0]} -->
              <StaffVoiceActingRoleCard type="staffMedia" source={edge} />
              <!-- {/if} -->
            {/each}
          {/each}
          <!-- Status data -->
          {#if isFetching}
            {#each Array(3) as noUse}
              <StaffVoiceActingRoleCardSkeleton />
            {/each}
          {:else if hasNextPage}
