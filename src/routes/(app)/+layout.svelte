<script lang="ts">
  ...
  let MediaDetailsModalWrap: typeof SvelteComponent;
  let ChracterDetailsModalWrap: typeof SvelteComponent;
  let StaffDetailsModalWrap: typeof SvelteComponent;
...
<div class="flex flex-col min-h-screen h-full w-full">
  <TopNavbar />
  <div class="flex flex-row w-full h-full mb-20">
    <SideNavbar />
    <!-- {#key $page.url} -->
    <div
      class="flex flex-col w-full h-full transition-[padding] duration-300"
      class:lg:pl-60={$isSideNavBar}
    >
      {#key mediaModal}
