<script lang="ts">
  ...
  import usePageUserFollowing from "@/hooks/usePageUserFollowing";
  ...
  const pageUserActivity = usePageUserFollowing({
    args: {
      userId: userId,
    },
  });
  ...
  onMount(() => {
    const unPageUserActivity = pageUserActivity.subscribe((value) => {
      ...
      const firstPage = pages?.[0]?.following;
      if (!!firstPage && !value.isFetchingNextPage) {
        if (cardProps.length) {
          const lastPageIndex = pages.length - 1;
          const edgesTemp = pages?.[lastPageIndex]?.following;
          if (edgesTemp) {
            cardProps = cardProps.concat(createCardProps(edgesTemp));
          }
        } else {
          for (const page of pages) {
            if (page?.following?.length) {
              const edgesTemp = page.following;
  ...
  const createCardProps = (following: Maybe<User>[]): CardUserProps[] => {
    const cardOptions: CardUserProps[] = [];
    for (const userData of following) {
