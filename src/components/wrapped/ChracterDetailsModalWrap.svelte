<script lang="ts">
  ...
  // Import Components
  import CharacterDetailsModal from "@/components/shared/CharacterDetailsModal.svelte";
  ...
  const characterDetails = useCharacterDetails({
    args: {
      id: options.value.id,
    },
  });
