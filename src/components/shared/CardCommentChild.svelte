<script lang="ts">
  ...
  interface CardChildernComment {
    userAvatar: string;
    userName: string;
    body: AnilistMarkdownHTML;
    createdAt: number;
    likeCount: number;
    isLocked: boolean;
    childComments: Maybe<CardChildernComment[]>;
  }
  ...
  export let userAvatar: CardChildernComment["userAvatar"];
  export let userName: CardChildernComment["userName"];
  export let body: CardChildernComment["body"];
  export let createdAt: CardChildernComment["createdAt"];
  export let likeCount: CardChildernComment["likeCount"];
  export let isLocked: CardChildernComment["isLocked"];
  export let childComments: CardChildernComment["childComments"];

  const timeAgo = formatTimeAgo(createdAt);
  const hasChildComments: boolean = !!childComments?.length;
  let isShowChildComment: boolean = true;

  const toggleShowComment = (): void => {
    isShowChildComment = !isShowChildComment;
  };
</script>

<div class="relative min-w-fit flex flex-col gap-y-1 pl-2 sm:rounded-lg">
  ...
  <div class="relative pl-9" class:comments-line={hasChildComments}>
    ...
    <div class="relative pt-1 flex gap-x-3 text-neutral-400 text-xs">
      ...
      {#if !isLocked}
        <button class="inline-flex items-center gap-x-2 action reply font-bold">
          <span>Reply</span>
        </button>
      {/if}
