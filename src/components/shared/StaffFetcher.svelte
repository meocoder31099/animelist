<script lang="ts">
  // Import Types
  import type { BrowseOptions } from "@/components/features/staff/StaffBrowseList.svelte";
  import type { CardStaffVerticalOptions } from "@/types";
  import type { Staff } from "@/types/anilist";

  // Import Libs
  import { onMount } from "svelte";
  import usePageStaff from "@/hooks/usePageStaff";
  ...
  onMount(() => {
    const unsubscribePageData = pageData.subscribe((value) => {
      ...
      const firstPage = pages?.[0]?.staff;
      if (!!firstPage && !value.isFetchingNextPage) {
        // Add staff to cardOptions
        if (cardOptions.length) {
          const lastPageIndex = pages.length - 1;
          const edgesTemp = pages?.[lastPageIndex]?.staff;
          if (edgesTemp) {
...
<VerticalStaffList {isLoading} {hasNextPage} source={cardOptions} />
