...
<div class="flex gap-3 md:gap-6 lg:gap-10 p-3 md:pt-4 md:pb-0 md:px-6 lg:px-12">
  ...
  <div class="w-full flex flex-col gap-y-2">
    {#if isMobile}
      <h1 class="text-xl font-bold line-clamp-2 mr-8 leading-6">{mediaTitle}</h1>
    {/if}
    {#if isLoading}
      <div
        class="w-full flex flex-col gap-y-2 mt-2 animate-pulse"
      >
        <!-- Stats  -->
        <div class="w-full h-5 bg-neutral-800 md:hidden rounded" />

        <!-- Descriptions -->
        <div class="w-full h-20 bg-neutral-800 md:bg-transparent rounded-lg" />
      </div>
    {:else if !!source}
      <!-- Info -->
      <div in:fade class="w-full flex flex-col gap-y-1 md:gap-y-4">
        <div class="w-full grid grid-cols-8 md:gap-x-10">
          <div class="col-span-full md:col-span-5 flex flex-col gap-2 md:gap-4">
            <div class="flex flex-col md:gap-1">
              ...
              {#if !!nextAiringEpisode}
                <div
                  class="inline-flex flex-wrap items-center gap-x-3 lg:gap-x-4 text-sm md:text-base 3xl:text-lg text-rose-500"
                >
