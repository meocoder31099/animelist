<script lang="ts">
  ...
  export let source: CardMediaVerticalOptions[];
...
<div
  class="w-full grid {gridColsAndGap ?? 'grid-cols-3 md:grid-cols-5 lg:grid-cols-6 2xl:grid-cols-7 3xl:grid-cols-8 gap-x-2 sm:gap-x-2.5 md:gap-x-2 lg:gap-x-3 2xl:gap-x-4 4xl:gap-x-5 gap-y-4 lg:gap-y-6'}"
>
  {#if !!source.length}
