<script lang="ts">
  ...
  const mediaHref = useCreateLink([
    "/",
    { value: source.node.type || "", convert: "lowcase" },
    "/details/",
    { value: source.node?.title?.userPreferred || "", convert: "-" },
    "-an",
    source.node?.id || 0,
  ]);

  const characterHref = useCreateLink([
    "/character/",
    { value: source.characters[0].name.userPreferred || "", convert: "-" },
    "-an",
    source.characters[0].id || 0,
  ]);
</script>
  ...
  <div class="w-full flex justify-between gap-1.5">
    <a
      use:onMediaDetailsModal={{
        ...
        bannerImage: source.node.bannerImage || "",
        href: mediaHref
      }}
      href={mediaHref}
